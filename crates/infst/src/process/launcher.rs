//! Game launcher — URI token extraction and game launch via token.
...
fn find_game_executable() -> anyhow::Result<std::path::PathBuf> {
...
/// Register the `bm2dxinf://` URI scheme in the Windows registry.
///
/// Sets up HKCU\Software\Classes\bm2dxinf to point to the current executable.
#[cfg(target_os = "windows")]
pub fn register_uri_scheme() -> anyhow::Result<()> {
    ...
    fn set_string_value(key: HKEY, name: Option<&str>, value: &str) -> anyhow::Result<()> {
        ...
        // SAFETY: RegSetValueExW writes a REG_SZ value to the registry. The PCWSTR
        // pointers remain valid for the duration of the call because the vecs are alive.
        unsafe {
            RegSetValueExW(
                key,
                pcwstr_name,
    ...
    let base = create_key(HKEY_CURRENT_USER, r"Software\Classes\bm2dxinf")?;
    set_string_value(base, None, "URL:bm2dxinf Protocol")?;
    set_string_value(base, Some("URL Protocol"), "")?;
