...
mod tests {
    ...
    /// Helper: build a mock memory reader with a custom miss_count value.
    /// Returns (reader, data_map_addr).
    fn build_single_node_reader_with_miss_count(miss_count: u32) -> (MockMemoryReader, u64) {
        let base = 0x1000u64;
        let data_map_addr = base + 16;
        let table_start = base + 32;
        let table_end = table_start + 8;
        ...
        node_bytes[24..28].copy_from_slice(&0i32.to_le_bytes()); // playtype = 0 (SP)
        node_bytes[32..36].copy_from_slice(&1500u32.to_le_bytes()); // score
        node_bytes[36..40].copy_from_slice(&miss_count.to_le_bytes()); // miss_count (custom)
        node_bytes[48..52].copy_from_slice(&4i32.to_le_bytes()); // lamp = Clear
    ...
    #[test]
    fn test_load_from_memory_miss_count_sentinel_becomes_none() {
        // u32::MAX is a sentinel value meaning "miss count unavailable"
        let (reader, data_map_addr) = build_single_node_reader_with_miss_count(u32::MAX);
        let song_db: HashMap<u32, SongInfo> = HashMap::new();
        let result = ScoreMap::load_from_memory(&reader, data_map_addr, &song_db).unwrap();
        assert_eq!(result.len(), 1);
        let score_data = result.get(1001).unwrap();
        assert_eq!(score_data.miss_count[0], None);
    }

    #[test]
    fn test_load_from_memory_miss_count_normal_value() {
        let (reader, data_map_addr) = build_single_node_reader_with_miss_count(42);
        let song_db: HashMap<u32, SongInfo> = HashMap::new();
        let result = ScoreMap::load_from_memory(&reader, data_map_addr, &song_db).unwrap();
        assert_eq!(result.len(), 1);
        let score_data = result.get(1001).unwrap();
        assert_eq!(score_data.miss_count[0], Some(42));
    }

    #[test]
    fn test_load_from_memory_miss_count_zero() {
        let (reader, data_map_addr) = build_single_node_reader_with_miss_count(0);
        let song_db: HashMap<u32, SongInfo> = HashMap::new();
        let result = ScoreMap::load_from_memory(&reader, data_map_addr, &song_db).unwrap();
        assert_eq!(result.len(), 1);
        let score_data = result.get(1001).unwrap();
        assert_eq!(score_data.miss_count[0], Some(0));
    }
