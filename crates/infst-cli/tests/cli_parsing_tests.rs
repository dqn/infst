...
fn test_parse_no_args() {
    let args = Args::try_parse_from(["infst"]).unwrap();
...
fn test_parse_find_offsets_with_output() {
    let args = Args::try_parse_from(["infst", "find-offsets", "-o", "custom.txt"]).unwrap();
    match args.command {
...
fn test_parse_status_with_json() {
    let args = Args::try_parse_from(["infst", "status", "--json"]).unwrap();
    match args.command {
...
fn test_parse_hexdump() {
    let args =
        Args::try_parse_from(["infst", "hexdump", "--address", "0x1000", "--size", "512"]).unwrap();
    match args.command {
...
fn test_parse_hexdump_with_ascii() {
    let args =
        Args::try_parse_from(["infst", "hexdump", "--address", "0x1000", "--ascii"]).unwrap();
    match args.command {
...
fn test_parse_offset() {
    let args =
        Args::try_parse_from(["infst", "offset", "--from", "0x1000", "--to", "0x2000"]).unwrap();
    match args.command {
...
fn test_parse_export_default_format() {
    let args = Args::try_parse_from(["infst", "export"]).unwrap();
    match args.command {
...
fn test_parse_export_json_format() {
    let args =
        Args::try_parse_from(["infst", "export", "-f", "json", "-o", "scores.json"]).unwrap();
    match args.command {
        Some(Command::Export { format, output, .. }) => {
...
fn test_parse_with_pid() {
    let args = Args::try_parse_from(["infst", "status", "--pid", "12345"]).unwrap();
    match args.command {
...
fn test_invalid_command_fails() {
    let result = Args::try_parse_from(["infst", "invalid-command"]);
